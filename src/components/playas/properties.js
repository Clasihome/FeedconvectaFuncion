...
export default ()=> {
  ...
  const getProperties = async ()=> {
    ...
    try{
      ...
      const params = location.search || `?status=PUBLICADA,ARRENDADA,VENDIDA&email=katy@morelpropiedades.cl&email=massiel@morelpropiedades.cl&address=Cerro%20Tacna`;
  ...
  const handlePaginate =(val)=> {
      ...
      const params = location.search || `?status=PUBLICADA,ARRENDADA,VENDIDA&email=katy@morelpropiedades.cl&email=massiel@morelpropiedades.cl`;
  ...
  return(
    <Section height="100vh">
      <Container>
        <Row>
          {
            query.data.properties.address.map((property) => (
